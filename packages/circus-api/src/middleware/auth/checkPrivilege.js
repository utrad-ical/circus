...
export default function checkPrivilege({ models }, route) {
	...
	return async function checkPrivilege(ctx, next) {
		...
		if (requiredGlobalPrivilege) {
			...
			const okay = requiredGlobalPrivilege.every(
		...
		if (requiredProjectPrivilege) {
			...
			if (project) {
				ok = requiredProjectPrivilege.every(
					rp => project.roles.some(r => r === rp)
				);
			} else {
				ok = false;
			}
			if (!ok) {
				ctx.throw(
					status.UNAUTHORIZED,
					`You do not have "${requiredProjectPrivilege.join(',')}" privilege of this project.`
				);
			}
		}
