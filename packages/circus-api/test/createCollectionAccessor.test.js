...
describe('createCollectionAccessor', function() {
  ...
  describe('#insert', function() {
    ...
    it('should raise an error on trying to insert invalid data', async function() {
      ...
      await asyncThrows(testCollection.insert({ month: 5 }), ValidationError);
      await asyncThrows(testCollection.insert({}), ValidationError);
    });
  });
  ...
  describe('#upsert', function() {
    ...
    it('should raise an error on trying to upsert invalid data', async function() {
      await asyncThrows(testCollection.upsert(3, { name: 3 }), ValidationError);
      await asyncThrows(testCollection.upsert(7, {}), ValidationError);
    });
  });
  ...
  describe('#modifyOne', function() {
    ...
    it('should throw if not found', async function() {
      await asyncThrows(async function() {
        const noSuchMonth = await testCollection.modifyOne(13, {
          $set: { name: 'Pon' }
        });
