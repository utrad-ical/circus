...
export async function checkTemporaryDirBase()
{
	...
	try {
		...
		if(! await isDir(temporaryDirBase))
			throw new Error (`config.temporaryDirBase: ${temporaryDirBase} is not exists.`);
							
		await mkDir(`${temporaryDirBase}/test`);
		await rmDir(`${temporaryDirBase}/test`);
	} catch(e) {
		throw new Error(`config.temporaryDirBase: ${temporaryDirBase} is something wrong.  (ex) not exists, no permission to write, ...etc`);
	}
}
...
{
	const {docker = {}} = config;
	const dockerSocketPath = docker.socketPath
...
export async function setupCoreMongo(): Promise<void>
{
	...
	try {
		...
		await db.collection("pluginJobQueue").createIndex({ jobId: 1 }, { unique: true });
		await client.close();
	} catch (e) {
		console.error(e);
	}
...
{
  const descriptor: {module: string, options: any} = config.dicomFileRepository;
