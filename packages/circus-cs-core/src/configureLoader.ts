...
import createDaemonController, { DaemonController } from './daemon/createDaemonController';
import { Configuration } from './config/Configuration';
import pluginJobRunner, { PluginJobRunner } from './job/pluginJobRunner';
import pluginDefinitionsAccessor, { PluginDefinitionAccessor } from './util/pluginDefinitionsAccessor';
import createPluginJobRegisterer, { PluginJobRegisterer } from './job/createPluginJobRegisterer';
...
export default function configureLoader(config: Configuration): DependentModuleLoader<CsModules> {
  const depLoader: DependentModuleLoader<CsModules> = new DependentModuleLoader();
  const disposeCollection: Dispose[] = [];
  depLoader.registerLoader('dispose', async () => {
    return async () => {
      await Promise.all(disposeCollection.map(i => i()));
    };
  }, []);
  depLoader.registerLoader('logger', async () => {
    return await loadModule<Logger>('logger', './logger', config.logger);
  }, []);
  depLoader.registerLoader('daemonController', async () => createDaemonController(config), []);
  depLoader.registerLoader('pluginDefinitionsAccessor', async () => pluginDefinitionsAccessor(config.coreWorkingDir), []);
  /**
   * Creates a MonboDB-based queue system based on the configuration.
   */
  depLoader.registerLoader('queueSystem', async (deps) => {
  ...
  depLoader.registerLoader('dicomFileRepository', async (deps) => {
    return await loadModule<DicomFileRepository>('dicom file repository', '@utrad-ical/circus-dicom-repository/lib', config.dicomFileRepository);
  }, []);
  depLoader.registerLoader('pluginJobRegisterer', async (deps) => {
  ...
  depLoader.registerLoader('jobReporter', async (deps) => {
  ...
  depLoader.registerLoader('jobRunner', async (deps) => {
