...
export default class DockerRunner extends EventEmitter {
	...
	public async run(createOptions: Dockerode.ContainerCreateOptions): Promise<string>
	{
		...
		while(1) {
			...
			// Check timeout
			if( this.timeout !== null && ((new Date()).getTime()) > (startTime + this.timeout) ) {
