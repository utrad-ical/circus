...
export default async function processNextJob(): Promise<boolean>
{
	...
	const queueItem: QueueSystem.Item<PluginJobRequest>|null = await QueueSystem.dequeue();
	if(queueItem === null)
		return false;
...
export async function processJob(queueItem: QueueSystem.Item<PluginJobRequest>): Promise<boolean>
{
	try {
		// 1. Mark "processing".
		...
		if(hook.proccessing)
			await hook.proccessing(queueItem.jobId);
		...
		var {
		  ...
		  tmpDicomDir,
		  tmpPluginInputDir,
		  tmpPluginOutputDir } = await createTemporaryDirectories(queueItem.jobId);
	} catch (e) {
	...
	try {
		...
		try {
			var pluginStdout = await executePlugin(payload.pluginId, tmpPluginInputDir, tmpPluginOutputDir);
		} catch (e) {
			//  Handle timeout, other unexpected errors.
			if( e instanceof DockerRunnerTimeout ) {
				if(hook.timeout)
					await hook.timeout(queueItem.jobId);
			} else {
				if(hook.failed)
					await hook.failed(queueItem.jobId, e.message, tmpPluginInputDir, tmpPluginOutputDir);
			}
			throw e;
		}
		...
		try {
			validatePluginExecutionResult(queueItem.jobId, pluginStdout, tmpPluginInputDir, tmpPluginOutputDir);
		} catch (e) {
			if(hook.invalidated)
				await hook.invalidated(queueItem.jobId, pluginStdout, tmpPluginInputDir, tmpPluginOutputDir, e);
			throw e;
		}
		...
		if(hook.finished)
			await hook.finished(queueItem.jobId, pluginStdout, tmpPluginInputDir, tmpPluginOutputDir);
...
export async function createTemporaryDirectories( jobId: string ): Promise<{tmpBaseDir: string, tmpDicomDir: string, tmpPluginInputDir: string, tmpPluginOutputDir: string}>
{
	...
	const tmpPluginInputDir = path.join( config.temporaryDirBase, directoryName, 'in');
	const tmpPluginOutputDir = path.join( config.temporaryDirBase, directoryName, 'out');
...
export async function fetchDICOMData(seriesUid: string, storeDir: string): Promise<void>
{
...
export async function parseDICOMData( srcDir: string, destDir: string ): Promise<[number, number, number]>
{
...
export async function executePlugin( pluginId: string, srcDir: string, destDir: string ): Promise<string>
{
	const {socketPath} = config.docker;
	...
	if(maxExecutionSeconds)
		runner.setTimeout(maxExecutionSeconds * 1000);
...
export async function validatePluginExecutionResult(jobId: string, result: string, tmpPluginInputDir: string, tmpPluginOutputDir: string): Promise<void>
{
