  ...
  implements Annotation, ViewerEventTarget, ModifierKeyBehaviors
{
  ...
  public draw(
    ...
    const min = convertVolumeCoordinateToScreenCoordinate(
      section,
      resolution,
      new Vector3(...this.min, this.z)
    );
    const max = convertVolumeCoordinateToScreenCoordinate(
      section,
      resolution,
      new Vector3(...this.max, this.z)
    );
  ...
  private hitTest(
    viewer: Viewer,
    point: Vector2,
    viewState: MprViewState | TwoDimensionalViewState
  ...
  public mouseMoveHandler(ev: ViewerEvent): void {
    ...
    const handleType = this.hitTest(viewer, point, viewState);
    if (handleType) {
      ev.stopPropagation();
