...
export default function fuzzySelect(
  ...
  const offsetBox = detectOffsetBox(rawData, startPoint, maxDistance, target);
  ...
  const binarizer = (p: Vector3) => {
    const value = rawData.getPixelAt(p.x, p.y, p.z);
...
function detectOffsetBox(
  ...
  const voxelSize = new Vector3(...rawData.getVoxelSize());

  const mprBoudaryOffsetBox = new Box3(
    new Vector3(0, 0, 0),
    new Vector3().fromArray(rawData.getDimension()).subScalar(1)
  );
