...
export const Tag = props => {
	...
	const color = tinycolor.mostReadable(backgroundColor, ['#000', '#fff']).toHexString();
	const borderColor = tinycolor(tagColor).darken().toHexString();
	const style = { color, backgroundColor, borderColor };
	return <span className="tag" style={style}>{props.name}</span>;
};
