				...
				<tbody>
					@foreach ($role_list as $role_key => $role_val)
						<tr>
							<th>{{$role_val}}</th>
							<td>
								{{Form::checkbox('priviledges.'.$role_key, 1, isset($inputs['priviledges_'.$role_key]) && $inputs['priviledges_'.$role_key] == 1 ? true : false)}}
								@if (isset($errors) && $errors->has('priviledges.'.$role_key))
									<br><span class="txt_alert">{{$errors->first('priviledges.'.$role_key)}}</span>
								@endif
							</td>
						</tr>
					@endforeach
