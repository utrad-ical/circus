@extends('common.layout')

@section('head')
{{HTML::script('js/jquery-ui.min.js')}}
<script>
	$(function(){
		$('#btnBack').click(function(){
			$(this).closest('div').find('.frmBack').submit();
			return false;
		});
	});
</script>
@stop

@section('title')
{{{Session::get('mode')}}} Case
@stop

@section('page_id')
id="page_case_input"
@stop

@section('content')
<div class="al_l mar_b_10">
	{{HTML::link(asset($back_url), $back_label, array('class' => 'common_btn', 'id' => 'btnBack'))}}
	{{Form::open(['url' => asset($back_url), 'method' => 'post', 'class' => 'frmBack'])}}
		{{Form::hidden('btnBack', 'btnBack')}}
	{{Form::close()}}
</div>
@if (isset($error_msg))
	<span class="text_error">{{nl2br($error_msg)}}</span>
@else
	{{Form::open(['url' => asset('case/complete'), 'method' => 'post', 'class' => 'frm_input'])}}
		<table class="common_table mar_b_10">
			<colgroup>
				<col width="10%">
				<col width="10%">
				<col width="40%">
				<col width="10%">
				<col width="30%">
			</colgroup>
			<tr>
				<th colspan="2">Case ID</th>
				<td colspan="3">{{$inputs['caseID']}}
					<span class="text_error">(ID is automatically generated by the system side)</span>
					@if (isset($errors) && $errors->has('caseID'))
						<br><span class="text_error">{{$errors->first('caseID')}}</span>
					@endif
				</td>
			</tr>
			<tr>
				<th colspan="2">Project ID</th>
				<td colspan="3">
					{{Form::select('projectID', $project_list, isset($inputs['projectID']) ? $inputs['projectID'] : '', array('common_input_select w_300'))}}
					@if (isset($errors) && $errors->has('projectID'))
						<br><span class="text_error">{{$errors->first('projectID')}}</span>
					@endif
				</td>
			</tr>
		@if(Auth::user()->hasPrivilege(Group::PERSONAL_INFO_VIEW))
			<tr>
				<th rowspan="2">Patient</th>
				<th>ID</th>
				<td>
					{{$inputs['patientInfoCache']['patientID']}}
					@if (isset($errors) && $errors->has('patientInfoCache_patientID'))
						<br><span class="text_error">{{$errors->first('patientInfoCahce_patientID')}}</span>
					@endif
				</td>
				<th>Age</th>
				<td>
					{{$inputs['patientInfoCache']['age']}}
					@if (isset($errors) && $errors->has('patientInfoCache_age'))
						<br><span class="text_error">{{$errors->first('patientInfoCache_age')}}</span>
					@endif
				</td>
			</tr>
			<tr>
				<th>Name</th>
				<td>
					{{$inputs['patientInfoCache']['patientName']}}
					@if (isset($errors) && $errors->has('patientInfoCache_patientName'))
						<br><span class="text_error">{{$errors->first('patientInfoCache_patientName')}}</span>
					@endif
				</td>
				<th>Sex</th>
				<td>
					{{CommonHelper::getSex($inputs['patientInfoCache']['sex'])}}
					@if (isset($errors) && $errors->has('patientInfoCache_sex'))
						<br><span class="text_error">{{$errors->first('patientInfoCache_sex')}}</span>
					@endif
				</td>
			</tr>
		@endif
			<tr>
				<th colspan="2">Series</th>
				<td colspan="3">
					<p>Please correct the order of the series.</p>
					<div id="series_order_wrap" class="w_500">
						<ul class="ui-sortable disp_series_list">
							@foreach($series_list as $key => $value)
								<li class="ui-state-dafault">
									<span>{{$value}}</span>
									<input type="hidden" value="{{$key}}" name="series[]" class="series_labels">
								</li>
							@endforeach
						</ul>
					</div>
					@if (isset($errors) && $errors->has('series'))
						<br><span class="text_error">{{$errors->first('series')}}</span>
					@endif
				</td>
			</tr>
		</table>
		<p class="al_c">
			{{Form::button('Confirmation', array('class' => 'common_btn btn_confirm'))}}
		</p>
	{{Form::close()}}

<div id="dialog" title="{{{Session::get('mode')}}} Case Confirmation" style="display: none;">
    <p class="mar_10">
        {{Form::open(['url' => asset('case/complete'), 'method' => 'post', 'class' => 'frm_confirm'])}}
			<table class="common_table mar_b_10">
				<colgroup>
					<col width="10%">
					<col width="10%">
					<col width="40%">
					<col width="10%">
					<col width="30%">
				</colgroup>
				<tr>
					<th colspan="2">Case ID</th>
					<td colspan="3">
						{{{$inputs['caseID']}}}
						<span class="text_error"><br>(ID is automatically generated by the system side)</span>
					</td>
				</tr>
				<tr>
					<th colspan="2">Project ID</th>
					<td colspan="3">
						{{Form::label('projectName','', array('class' => 'view_project'))}}
					</td>
				</tr>
				@if(Auth::user()->hasPrivilege(Group::PERSONAL_INFO_VIEW))
					<tr>
						<th rowspan="2">Patient</th>
						<th>ID</th>
						<td>{{$inputs['patientInfoCache']['patientID']}}</td>
						<th>Age</th>
						<td>{{$inputs['patientInfoCache']['age']}}</td>
					</tr>
					<tr>
						<th>Name</th>
						<td>{{$inputs['patientInfoCache']['patientName']}}</td>
						<th>Sex</th>
						<td>{{CommonHelper::getSex($inputs['patientInfoCache']['sex'])}}</td>
					</tr>
				@endif
				<tr>
					<th colspan="2">Series</th>
					<td colspan="3">
						<span class="view_series"></span>
					</td>
				</tr>
			</table>
			<p class="submit_area">
				{{Form::button('Cancel', array('class' => 'common_btn btn_cancel'))}}
				{{Form::button('Save', array('class' => 'common_btn btn_save'))}}
			</p>
		{{Form::close()}}
    </p>
</div>
@endif
<script>
var post_data;
$(function(){
	$('.btn_confirm').click(function() {
		var parent_form = $(this).closest('form');
		var target_form = $('.frm_confirm');

		//書き出し処理
		//プロジェクト
		var project_name = parent_form.find('select[name="projectID"] option:selected').text();
		target_form.find('.view_project').empty();
		target_form.find('.view_project').text(project_name);

		//シリーズ
		var series = parent_form.find('.ui-state-dafault').find('span').get();
		target_form.find('.view_series').empty();
		var label_series = new Array();
		jQuery.each(series, function(){
			target_form.find('.view_series').append('<p>'+$(this).text()+'</p>');
		});
		createConfirmDialog();
	});
	$('.btn_cancel').click(function() {
		closeConfirmDialog();
	});
	$('.btn_save').click(function() {
		$('.frm_input').submit();
		return false;
	});
});
var closeConfirmDialog = function() {
	$("#dialog").dialog('close');
}
var createConfirmDialog = function() {
	$('#dialog').slideDown();
	$('#dialog').dialog('open');
}
$("#dialog").dialog({
	autoOpen: false,
	closeOnEscape: false,
	closeText:"",
	width:750,
	maxwidth:false,
	modal:true
});
</script>
@stop