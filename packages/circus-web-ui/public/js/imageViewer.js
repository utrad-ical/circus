...
;(function($){
 ...
 $.widget('ui.imageViewer', { // ビューアーごとオプション、デフォルト値
  ...
  drawLabel : function (label_id,positions_array){
   //塗り機能
   //第1引数 : 対象ラベル
   ...
   //描画対象ラベルのチェック
   var target_label = this_obj._getLabelObjById(label_id);
  ...
  eraseLabel : function (label_id,positions_array){
   //塗り機能
   //第1引数 : 対象ラベル
  ...
  _getLabelObjById : function(label_id){
   //描画対象ラベルのチェック
   var this_opts = this.options;
    for(var i=this_opts.viewer.draw.label.length-1; i>=0; i--){
    if(this_opts.viewer.draw.label[i].id == label_id){
     return this_opts.viewer.draw.label[i];
    }
    }
  },
  ...
  syncVoxel : function(){
   ...
   //全ラベルについて現在の[オリエンテーション・奥行] を渡して塗るべき座標を戻してもらう
   for(var i=this_opts.control.container.data.label.length-1; i>=0; i--){

    var tmp_array = this_opts.control.container.returnSlice(
            this_opts.control.container.data.label[i].id,
            this_opts.viewer.orientation,
            this_opts.viewer.number.current
           );
    if(tmp_array.length>0){
     this_obj.drawLabel(this_opts.control.container.data.label[i].id,tmp_array);
    }
   }
  }/*syncVoxel*/,
