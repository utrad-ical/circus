...
const Candidate = React.forwardRef((props, ref) => {
  ...
  const applyDisplayOptions = useCallback(
    ...
    [item.volumeId, job.results.metadata]
  );

  const initialStateSetter = useCallback(
    (viewer, state) => applyDisplayOptions(centerState(state)),
    [applyDisplayOptions, centerState]
  );
